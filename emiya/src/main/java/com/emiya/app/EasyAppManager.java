 ...
 * @author liminghuang
 */
public final class EasyAppManager {

    // 使用弱引用是因为存在未使用AppManager的finish方法来释放的activity，但mActivityStack并未断开对其应用导致内存泄露的问题
    private Stack<WeakReference<Activity>> mActivityStack;
    private static volatile EasyAppManager sInstance;

    private EasyAppManager() {
    }
     ...
     */
    public static EasyAppManager getInstance() {
        if (sInstance == null) {
            synchronized (EasyAppManager.class) {
                if (sInstance == null) {
                    sInstance = new EasyAppManager();
                }
            }
        }
        return sInstance;
